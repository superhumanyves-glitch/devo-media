import Header from "@/components/Header";
import Footer from "@/components/Footer";
import { SEO } from "@/components/SEO";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/ui/accordion";
import { useTranslation } from "react-i18next";

interface TermsSection {
  title: string;
  items?: string[];
  paragraph?: string;
  definitions?: boolean;
}

const AlgemeneVoorwaarden = () => {
  const { t } = useTranslation();
  const sections = t("legal.terms.sections", { returnObjects: true }) as TermsSection[];
  const definitions = t("legal.terms.definitions", { returnObjects: true }) as { term: string; text: string }[];

  return (
    <div className="min-h-screen flex flex-col">
      <SEO
        title={t("legal.terms.seoTitle")}
        description={t("legal.terms.seoDescription")}
        canonicalUrl="/algemene-voorwaarden"
      />
      <Header />

      <main className="flex-grow bg-background pt-24">
        <div className="container mx-auto px-4 py-12 max-w-4xl text-left">
          <h1 className="text-3xl md:text-4xl font-bold mb-3 text-foreground text-left">{t("legal.terms.title")}</h1>
          <p className="text-muted-foreground mb-8 text-left">{t("legal.lastUpdate")}</p>

          <div className="bg-card/50 backdrop-blur-sm p-6 rounded-xl border border-border/50 shadow-sm mb-6">
            <h2 className="text-lg font-semibold mb-3 text-foreground text-left">{t("legal.companyHeading")}</h2>
            <div className="text-sm text-muted-foreground space-y-1">
              <p><span className="font-medium text-foreground">{t("legal.addressLabel")}:</span> {t("legal.address")}</p>
              <p><span className="font-medium text-foreground">{t("legal.kvkLabel")}:</span> {t("legal.kvk")}</p>
              <p><span className="font-medium text-foreground">{t("legal.vatLabel")}:</span> {t("legal.vat")}</p>
            </div>
          </div>

          <Accordion type="single" collapsible className="space-y-4">
            {sections.map((section, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="bg-card/30 backdrop-blur-sm rounded-xl border border-border/50 px-6">
                <AccordionTrigger className="text-base font-semibold text-foreground hover:no-underline text-left">
                  {section.title}
                </AccordionTrigger>
                <AccordionContent className="text-sm text-muted-foreground space-y-2 pt-2">
                  {section.definitions && (
                    <>
                      <p>{t("legal.terms.definitionsIntro")}</p>
                      <ul className="space-y-2">
                        {definitions.map((def, i) => (
                          <li key={i}>
                            <span className="font-medium text-foreground">{def.term}</span> {def.text}
                          </li>
                        ))}
                      </ul>
                    </>
                  )}
                  {section.paragraph && <p>{section.paragraph}</p>}
                  {section.items && (
                    <ul className="space-y-2">
                      {section.items.map((item, i) => (
                        <li key={i}>• {item}</li>
                      ))}
                    </ul>
                  )}
                </AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>

          <div className="bg-muted/30 backdrop-blur-sm p-4 rounded-xl border border-border/50 mt-8">
            <p className="text-xs text-muted-foreground">
              {t("legal.rightsReserved")}
            </p>
          </div>
        </div>
      </main>

      <Footer />
    </div>
  );
};

export default AlgemeneVoorwaarden;
